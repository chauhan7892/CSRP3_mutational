...
def stability_check(data_ori):
	...
	data_expmt = (0.078 + data_ori)/1.14
	# stability criterion
	if (data_expmt > 3):
		return(stab_list[0]) # highly de-stabilizing
	elif (data_expmt <= 3) & (data_expmt > 2):
		return(stab_list[1]) # de-stabilizing
	elif (data_expmt <= 2) & (data_expmt > 1):
		return(stab_list[2]) # slightly de-stabilizing
	elif (data_expmt <= 1) & (data_expmt > -1.0):
		return(stab_list[3]) # neutral
	elif (data_expmt <= -1.0) & (data_expmt > -2.0):
		return(stab_list[4]) # slightly stabilizing
	elif (data_expmt <= -2.0) & (data_expmt > -3.0):
