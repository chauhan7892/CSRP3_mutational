		...
		if root !=repair_pdb_dir:			
			for file_name in files:
				...
				if os.path.exists(output_dir_phenotype):					
					print('foldx_data already exists : trying to remove')
					shutil.rmtree(output_dir_phenotype,ignore_errors = False)
					print(output_dir_phenotype)
					os.mkdir(output_dir_phenotype)
				else:
					os.mkdir(output_dir_phenotype)

				# parameters for single_task_foldx function
				params = zip(foldx_exec, repair_pdb_dir, repair_pdb_name, the_mutant_file, output_dir_phenotype)								
